...
<template>
  <ul class="divide-y-4 lg:w-2/3 xl:w-1/2">
    <li class="cursor-pointer py-4" v-for="(day, index) in scheduleData">
      <div class="flex items-center justify-between px-4" @click="accordionState[index] = !accordionState[index]">
        <p
          class="text-2xl font-semibold first-letter:uppercase"
          :class="weekdayNumber.getDay() === correctDayId(day._id) && 'text-primary'"
        >
